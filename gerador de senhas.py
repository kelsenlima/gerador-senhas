...
class GUI():
    def __init__(self,master):
        ...
        self.label = Label(text="Gerador de Senhas", anchor=N, fg='darkblue', font='arial 20 bold underline')
        ...
        self.user = Label(text="Qual é seu nome: ", font='times 15')
        ...
        self.length = Label(text="Qual será o comprimento da senha: ", font='times 15')
        ...
        self.generated_password = Label(text="Senha gerada: ", font='times 15')
        ...
        self.generate = Button(text="GERAR SENHA", bd=3, relief='solid', padx=1, pady=1, font='times 15', fg='white', bg='darkblue', command=self.generate_pass)
        self.generate.grid(row=11, column=1)

        self.blank_label5 = Label(text="")
        self.blank_label5.grid(row=12, column=0)

        self.accept = Button(text="ACEITAR", bd=3, relief='solid', padx=1, pady=1, font='times 15', fg='darkblue', bg='white', command=self.accept_fields)
        self.accept.grid(row=13, column=1)

        self.blank_label1 = Label(text="")
        self.blank_label1.grid(row=14, column=1)

        self.reset = Button(text="LIMPAR", bd=3, relief='solid', padx=1, pady=1, font='times 15', fg='darkblue', bg='white', command=self.reset_fields)
    ...
    def generate_pass(self):
        ...
        if name.isalpha()==False:
            messagebox.showerror("Erro","valor incorreto")
    ...
    def accept_fields(self):
        with sqlite3.connect("banco.db") as db:
            ...
            if cursor.fetchall():
            	messagebox.showerror("Esse nome de usuário já existe!", "Use outro nome de usuário")
            else:
            	insert = str("INSERT INTO users(Username, GeneratedPassword) VALUES(\'%s\', \'%s\');"%(self.n_username.get(), self.n_generatedpassword.get()))
            	cursor.execute(insert)
            	db.commit()
            	messagebox.showinfo("Sucesso!", "Senha gerada com sucesso")
